...
{
    class SdkStyleVsProjectLoader : IVsProjectLoader
    {
        public VsProject Load(string projectPath)
        {
            Contract.RequiresNotNullNotEmpty(projectPath, nameof(SdkStyleVsProjectLoader));
        ...
        private static string GetAssemblyNameForProject(string projectPath, XDocument projDoc)
        {
            ...
            {
                var assemblyNodes = projDoc.Descendants("TargetName");
                if (assemblyNodes.Count() != 0)
                {
                    return assemblyNodes.First().Value;
                }

                assemblyNodes = projDoc.Descendants("ProjectName");
                if (assemblyNodes.Count() != 0)
                {
                    return assemblyNodes.First().Value;
                }
            ...
            {
                var assemblyNodes = projDoc.Descendants("AssemblyName");
                if (assemblyNodes.Count() != 0)
                {
                    return assemblyNodes.First().Value;
                }
        ...
        {
            var contentNodeLists = projDoc.Descendants("Content");
            ...
            foreach (XElement contentNode in contentNodeLists)
            {
                if (contentNode.HasAttributes &&
                    ...
                    contentNode.Elements("CopyToOutputDirectory").Any())
                {
        ...
        {
            var configNodeLists = projDoc.Descendants("None");
        ...
        {
            return projDoc.Descendants("Reference")
                .Select(x => x.Attribute("Include").Value).ToList();
        }

        private static bool IsAssemblySerializable(XDocument ProjDoc)
        {
            var assemblySerializableNode = ProjDoc.Descendants("GenerateSerializationAssemblies");
        ...
        {
            var projOutputTypeNode = projDoc.Descendants("OutputType");
