...
public class LogParserTests
{
    ...
    {
        var reader = new StringReader(@"C:\ws\Plainion.GraphViz\src\Plainion.GraphViz.ActorsHost\Program.cs(53,13): error CA1031: Modify 'Main' to catch a more specific allowed exception type, or rethrow the exception (https://learn.microsoft.com/dotnet/fundamentals/code-analysis/quality-rules/ca1031) [C:\ws\plainionist\Plainion.GraphViz\src\Plainion.GraphViz.ActorsHost\Plainion.GraphViz.ActorsHost.csproj]\r\n");
        var parser = new LogParser();
        ...
        Assert.That(result[0].File, Is.EqualTo(@"C:\ws\Plainion.GraphViz\src\Plainion.GraphViz.ActorsHost\Program.cs"));
    ...
    public void Parse_ShouldReturnMultipleCompilerAlerts_WhenReaderContainsMultipleAlerts()
    {
        ...
        Assert.That(result[0].File, Is.EqualTo(@"C:\ws\Plainion.GraphViz\src\Plainion.GraphViz\Dot\DotPlainReader.cs"));
        Assert.That(result[0].LineNumber, Is.EqualTo(105));
        Assert.That(result[0].Code, Is.EqualTo("CA2201"));

        Assert.That(result[1].File, Is.EqualTo(@"C:\ws\Plainion.GraphViz\src\Plainion.GraphViz\Dot\DotWriter.cs"));
